...
class NTFS:
  ...
  def __init__(self, name: str) -> None:
    ...
    try:
      ...
      raise Exception(f"No volume named {name}")
    except PermissionError:
      print("[ERROR] Permission denied, try again as admin/root")
      raise Exception("Permission denied, try again as admin/root")
  ...
  def GetInformation(self) -> dict[str, str]:
    res = {}
    res["Name"] = self.name
    for key in NTFS.important_info:
      res[key] = self.boot_sector[key]
    return res
